...
```{r setting}
setwd("~/Desktop/DMJS7 Dissertation")
```

## Asylum application from 2010 to 2022
```{r intro}
# Importing asylum application and caseload data
asylum_data <- read.csv("Home Office Data.csv")
asylum_data$year <- as.factor(asylum_data$year)
asylum_data$awaiting_initial <- as.numeric(asylum_data$awaiting_initial)
asylum_data$appeal <- as.numeric(asylum_data$appeal)
asylum_data$removal <- as.numeric(asylum_data$removal)
asylum_data_long <- reshape2::melt(asylum_data, id.vars = "year") %>%
  subset(variable %in% c("awaiting_initial", "appeal", "removal"))
asylum_data_long$variable <- factor(asylum_data_long$variable, levels = rev(unique(asylum_data_long$variable)))

# Visualising trend
library(ggplot2)
ggplot() +
  geom_bar(data = asylum_data_long, aes(x = year, y = value, fill = variable),
           stat = "identity", position = "stack") +
  geom_line(data = asylum_data, aes(x = year, y = new_applications, group = 1, color = "New Applications")) +
  scale_fill_manual(values = c("#8080C2", "#96C9EB", "#66BBBB"),
                    labels = c("Subject to Removal", "Appeal outstanding","Awaiting initial decision")) +
  labs(x = "Year (ending June)",
       y = "Thousands",
       fill = "Type") +
  scale_color_manual(values = "black", name = "New Applications") + 
...
1) Customised Script for Extracting Speeches and Attributes
```{r 2}
library(XML)
library(stringi)

# The following customised functions was inspired by Watanabe's (2014) script
  ...
  for (node in getNodeSet(xml, "//*")) {
    if (xmlName(node) %in% c("major-heading","minor-heading")) {
      ...
      keywords <- c("asylum", "refugee", "english channel", "channel crossing", 
                    ...
                    "Afghan Citizens Resettlement Scheme", "Afghan Resettlement Update", 
...
mp_party <- read.csv("MPs Election Data 2019.csv") # 2019
mp_party_2017 <- read.csv("MPs Election Data 2017.csv") # 2017  
mp_party_2015 <- read.csv("MPs Election Data 2015.csv") # 2015
...
3) Loading Debate Data + Merging with MP data + Filling Missing Data 
```{r 4, message=FALSE, results="hide"}
##### 2023 merged debate data #####
library(dplyr)
debate_2023 <- readDebateDir("~/Desktop/DMJS7 Dissertation/XML Datasets/xml_2023/debates/")
...
debate_2022 <- readDebateDir("~/Desktop/DMJS7 Dissertation/XML Datasets/xml_2022/debates/")
...
debate_2021 <- readDebateDir("~/Desktop/DMJS7 Dissertation/XML Datasets/xml_2021/debates/")
...
debate_2020 <- readDebateDir("~/Desktop/DMJS7 Dissertation/XML Datasets/xml_2020/debates/")
...
debate_2019 <- readDebateDir("~/Desktop/DMJS7 Dissertation/XML Datasets/xml_2019/debates/")
...
debate_2018 <- readDebateDir("~/Desktop/DMJS7 Dissertation/XML Datasets/xml_2018/debates/")
...
debate_2017 <- readDebateDir("~/Desktop/DMJS7 Dissertation/XML Datasets/xml_2017/debates/")
...
debate_2016 <- readDebateDir("~/Desktop/DMJS7 Dissertation/XML Datasets/xml_2016/debates/")
...
debate_2015 <- readDebateDir("~/Desktop/DMJS7 Dissertation/XML Datasets/xml_2015/debates/")
...
4) Cleaning Dataset and Saving as a CSV file 
```{r 5}
...
nrow(debate_dataset) # Total Number of Speeches in Cleaned Dataset: 7411
```
...
4) Number of Words Per Speech (BEFORE TEXT CLEANING)
```{r b4}
library(stringr)
...
# Mapping MPs' contributions (UK)
shapefile <- st_read("~/Desktop/DMJS7 Dissertation/UK Constituency Shapefiles/UK Constituency.shp")
region_shapefile <- st_read("~/Desktop/DMJS7 Dissertation/UK Regional Boundary Shapefiles/UK Regional Boundary.shp")
...
t$total_count <- as.numeric(t$total_count)
t$total_count <- ifelse(is.na(t$total_count), 0, t$total_count)

tm_shape(t) +
  tm_fill(title = "Count", 
          palette = c("#fcfeff","#bdd7e7", "#6baed6", "#3182bd", "#08519c","#000080"), 
          fill = "total_count",
          border.alpha = 0.9,
          midpoint = NA,
          breaks = c(0, 1, 5, 10, 50, 400)) +  
  tm_borders(col = "#FFFFFF", lwd = 0.3) +
  tm_shape(region_sh) +
...
# Histogram
breaks <- c(0, 1, 5, 10, 50, 400)
colors <- c("#fcfeff", "#bdd7e7", "#6baed6", "#3182bd", "#08519c", "#000080")
t$color_group <- cut(t$total_count, breaks = c(-Inf, breaks, Inf), labels = FALSE)

ggplot(data = t, aes(x = total_count, fill = factor(color_group))) +
  geom_histogram(binwidth = 3) +
  geom_histogram(data = subset(t, total_count == 0), binwidth = 3, 
                 fill = "#fcfeff",col="#71797E") + 
  scale_fill_manual(values = c("#fcfeff", "#bdd7e7", "#6baed6", "#3182bd", 
                                        "#08519c", "#000080")) +
                                          labs(title = " ", x = " ", y = " ") +
 theme(axis.text.y = element_text(face = "bold"),
        axis.text.x = element_text(face = "bold", angle = 90, vjust=0.4),
        axis.line = element_line(colour = "black"),
        panel.background = element_blank())+
  scale_x_break(c(50, 50), scales = 0.3,space = 0.1)+
...
london$total_count <- as.numeric(london$total_count)
london$total_count <- ifelse(is.na(london$total_count), 0, london$total_count)
...
```{r e6}
# Biographical data
biodata2015 <- read_sav("MPs Biographical Data 2015.sav")
biodata2017 <- read_sav("MPs Biographical Data 2017.sav")
biodata2019 <- read_sav("MPs Biographical Data 2019.sav")
...
missing_data <- read.csv("Manually Filled Missing Data.csv")
...
region <- read.csv("Region from Constituency Conversion.csv")
...
table(debate_dataset$speaker_name[debate_dataset$Party=="Conservative"]) # Most contributing Conservative MPs
...
table(debate_dataset$speaker_name[debate_dataset$Party=="Labour"]) # Most contributing Labour MPs
...
```{r f2-a}
# Plotting Results From Python (see Python script from supplementary file)
...
# Convert predicted_label to character strings
...
# Creating a dataset with the counts of document labels by refugee type
...
label_counts_con$refugee <- factor(label_counts_con$refugee, levels = desired_order) 
...
lib_type <- dataset_bytype[dataset_bytype$Party=="Liberal Democrats",]
label_counts_lib <- lib_type %>%
  group_by(refugee, predicted_label) %>%
  summarise(count = n()) 
label_counts_lib$predicted_label <- ifelse(label_counts_lib$predicted_label == 1, "Hostility", "Hospitality")
label_counts_lib$refugee <- factor(label_counts_lib$refugee, levels = desired_order) # Reorder the refugee column

label_counts_lib <- label_counts_lib %>%
  group_by(refugee) %>%
  mutate(proportion = count / sum(count))

lib <- ggplot(label_counts_lib, aes(x = refugee, 
                                    y = count, 
                                    fill = predicted_label)) +
  geom_bar(stat = "identity") +
  geom_text(data = filter(label_counts_lib, predicted_label == "Hospitality"),
            aes(label = paste0(round(proportion * 100, 1), "%")), 
            vjust = 0.3, size = 0, color = "#3F4547") +  # Add proportion labels
  labs(title = "Liberal Democrats", x = " ",
...
snp_type <- dataset_bytype[dataset_bytype$Party=="SNP",]
label_counts_snp <- snp_type %>%
  group_by(refugee, predicted_label) %>%
  summarise(count = n()) 
label_counts_snp$predicted_label <- ifelse(label_counts_snp$predicted_label == 1, "Hostility", "Hospitality")
label_counts_snp$refugee <- factor(label_counts_snp$refugee, levels = desired_order) # Reorder the refugee column

label_counts_snp <- label_counts_snp %>%
  group_by(refugee) %>%
  mutate(proportion = count / sum(count))

snp <- ggplot(label_counts_snp, aes(x = refugee, 
                                    y = count, 
                                    fill = predicted_label)) +
  geom_bar(stat = "identity") +
  geom_text(data = filter(label_counts_snp, predicted_label == "Hospitality"),
            aes(label = paste0(round(proportion * 100, 1), "%")), 
            vjust = 0.3, size = 0, color = "#3F4547") +  # Add proportion labels
  labs(title = "SNP", x = " ",
       y = " ", fill = "Speech Category") +
...
grid.arrange(con, lab, lib, snp, nrow = 2)
```
...
type_tfidf <- TypeCorpus %>% 
  ...
  tokens_wordstem() %>% 
  dfm(tolower=T) %>% # dfm
  dfm_trim(min_docfreq = 5, docfreq_type = "count") %>% # remove words appearing 5 times or fewer
  dfm_trim(max_docfreq = 0.95, docfreq_type = "prop") %>% 
...
# Fixing Political Party Names 
mp_party$Party <- ifelse(mp_party$Party == "Scottish National Party", "SNP",
                         ifelse(mp_party$Party == "Social Democratic and Labour Party", "SDLP",
                                      ifelse(mp_party$Party == "Liberal Democrat", 
                                             "Liberal Democrats", mp_party$Party)))
mp_party$Party <- ifelse(mp_party$Party == "Labour/Co-operative","Labour", mp_party$Party)
mp_party$Party <- ifelse(mp_party$Party %in% c("Alba", "Alliance", "Plaid Cymru", 
                                               "SDLP","Speaker","Sinn Féin"), 
                         "Others", mp_party$Party)

party_colors <- c("Conservative" = "#239ade","Labour" = "#c7223a",
                  "Liberal Democrats" = "#feb01f", "SNP" = "#8B8000",
                  "Green"="darkgreen", "Others"="purple3","Independent"="pink2",
                  "DUP"="#DE3163","Sinn Féin"="green2")

region_sh <- st_simplify(region_shapefile, preserveTopology = TRUE, dTolerance = 1000)

# 2015 Election
mp_party_2015$Party <- ifelse(mp_party_2015$Party == "Scottish National Party", "SNP",
                              ifelse(mp_party_2015$Party == "Social Democratic and Labour Party", "SDLP",
                                     ifelse(mp_party_2015$Party == "Liberal Democrat", 
                                            "Liberal Democrats", mp_party_2015$Party)))
mp_party_2015$Party <- ifelse(mp_party_2015$Party == "Labour/Co-operative","Labour", mp_party_2015$Party)
mp_party_2015$Party <- ifelse(mp_party_2015$Party %in% c("Alba", "Alliance", "Plaid Cymru", 
                                                         "SDLP","Speaker","Sinn Féin","UKIP","UUP"), 
                              "Others", mp_party_2015$Party)
table(mp_party_2015$Party)

# 2017 Election
mp_party_2017$Party <- ifelse(mp_party_2017$Party == "Scottish National Party", "SNP",
                              ifelse(mp_party_2017$Party == "Social Democratic and Labour Party", "SDLP",
                                     ifelse(mp_party_2017$Party == "Liberal Democrat", 
                                            "Liberal Democrats", mp_party_2017$Party)))
mp_party_2017$Party <- ifelse(mp_party_2017$Party == "Labour/Co-operative","Labour", mp_party_2017$Party)
mp_party_2017$Party <- ifelse(mp_party_2017$Party %in% c("Alba", "Alliance", "Plaid Cymru", 
                                                         "SDLP","Speaker","Sinn Féin","UKIP","UUP"), 
                              "Others", mp_party_2017$Party)
table(mp_party_2017$Party)
...
mp_party$Party <- ifelse(mp_party$Party %in% c("Alba", "Alliance", "Plaid Cymru", 
                                               "SDLP","Speaker","Sinn Féin"), 
                         "Others", mp_party$Party)
table(mp_party$Party)

# 2015 Map 

political_map_2015 <- merge(shapefile, 
                            mp_party_2015, 
                       by.x = "PCON21NM",  
                       by.y = "Constituency", 
                       all.x = TRUE)
political_map_2015 <- st_simplify(political_map_2015, preserveTopology = TRUE, dTolerance = 1000)

political_map_2015$Party[political_map_2015$PCON21NM == "Mid Bedfordshire"] <- "Conservative"
political_map_2015$Party[political_map_2015$PCON21NM == "Selby and Ainsty"] <- "Conservative"
political_map_2015$Party[political_map_2015$PCON21NM == "Uxbridge and South Ruislip"] <- "Conservative"

library(tmap)
tm_shape(political_map_2015) +
  tm_fill(title = "Party", 
          fill = "Party",
...
political_map_2017 <- merge(shapefile, 
                       mp_party_2017, 
                       by.x = "PCON21NM",  
                       by.y = "Constituency", 
                       all.x = TRUE)
political_map_2017 <- st_simplify(political_map_2017, preserveTopology = TRUE, dTolerance = 1000)

tm_shape(political_map_2017) +
  tm_fill(title = "Party", 
          fill = "Party",
          palette = party_colors, 
          border.alpha = 1,
...
political_map <- merge(shapefile, 
                       mp_party, 
                       by.x = "PCON21NM",  
                       by.y = "Constituency", 
                       all.x = TRUE)
political_map <- st_simplify(political_map, preserveTopology = TRUE, dTolerance = 1000)

political_map$Party[political_map$PCON21NM == "Mid Bedfordshire"] <- "Conservative"
political_map$Party[political_map$PCON21NM == "Selby and Ainsty"] <- "Conservative"
political_map$Party[political_map$PCON21NM == "Uxbridge and South Ruislip"] <- "Conservative"

tm_shape(political_map) +
  tm_fill(title = "Party", 
          fill = "Party",
          palette = party_colors,
          border.alpha = 1,
